...
class VamtamSidebars {
	...
	public function __construct() {
		$this->sidebars = array(
			'page' => esc_html__( 'Main Widget Area', 'nex' ),
		);

		if ( vamtam_has_woocommerce() )
			$this->sidebars['vamtam-woocommerce'] = esc_html__( 'WooCommerce Widget Area', 'nex' );
	...
	public function register_sidebars() {
		...
		register_sidebar( array(
			...
			'name'          => __( 'Overlay Menu Widget Area', 'nex' ),
			'description'   => __( 'Displayed below the menu items in the overlay menu.', 'nex' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		) );
	}
