...
<div class="team-member <?php echo ( ! empty( $biography ) ? 'has-content' : '' ) ?>">
			...
			<?php if ( ! empty( $used_icons ) ): ?>
				<div class="share-icons clearfix">
					<?php foreach ( $used_icons as $icon ) : ?>
						<a href="<?php echo esc_url( $icon['url'] )?>" title=""><?php echo vamtam_get_icon_html( array( // xss ok
							'name' => $icon['name'],
						) ); ?></a>
					<?php endforeach; ?>
				</div>
	...
	<div class="team-member-info">
		...
		<?php if ( ! empty( $settings->phone ) ) : ?>
			<div class="team-member-phone"><a href="tel:<?php echo esc_url( $settings->phone ) ?>" title="<?php echo esc_attr( sprintf( 'Call %s', $settings->name ) ) ?>"><?php esc_html_e( 'Tel:', 'nex' ) ?> <?php echo wp_kses_post( $settings->phone ) ?></a></div>
		<?php endif ?>
		<?php if ( ! empty( $settings->email ) ) : ?>
			<div  class="team-member-email"><a href="mailto:<?php echo esc_attr( $settings->email ) ?>" title="<?php echo esc_attr( sprintf( __( 'email %s', 'nex' ), $settings->name ) ) ?>"><?php echo wp_kses_post( $settings->email ); ?></a></div>
		<?php endif ?>
