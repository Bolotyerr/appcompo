...
$myrows = $wpdb->get_results( "SELECT * FROM $table_builder LIMIT 0, 40" );
$mysub = $wpdb->get_results( "SELECT * FROM $table_subs LIMIT 0, 40", 'ARRAY_A' );
$mysubr = $wpdb->get_results( "SELECT * FROM $table_subs WHERE seen='1' LIMIT 0, 40", 'ARRAY_A' );
									...
									<tbody>
										...
										foreach ($myrows as $row) {
											...
											<tr id='<?php echo $row->id; ?>'>
												<td class='row_click' style='text-align: center'><a href='admin.php?page=survey_builder&id=<?php echo $row->id; ?>'><?php echo $row->id; ?></a></td>
												...
												<td style='text-align: center; border-right: 1px solid #eee'>
													...
													<a style='width: 36px; float: right; box-sizing: border-box' class='delete-row btn btn-danger' data-loading-text='...' data-complete-text="<i class='icon-ok icon-white'></i>" id='delete_<?php echo $row->id; ?>' title='Delete this form'><i class='icon-remove icon-white'></i>
													</a>
													<a href='<?php echo $exp.$row->id; ?>' class='btn btn-success' target='_blank' title='Export all submissions for this form' style='float: right; box-sizing: border-box; margin-right: 7px;'><i class='icon-share-alt icon-white'></i> </a>
												</td>
											</tr>
											<?php } ?>
