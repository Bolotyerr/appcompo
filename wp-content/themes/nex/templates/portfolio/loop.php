...
if ( 'ajax' === $settings->link_opens ) {
	$cube_options = array_merge( $cube_options, array(
		...
		'singlePageCounter'          => '<div class="cbp-popup-singlePage-counter">' . esc_html__( '{{current}} of {{total}}', 'nex' ) . '</div>',
...
<section class="<?php echo esc_attr( implode( ' ', $wrapper_class ) ) ?>" id="<?php echo esc_attr( $main_id ) ?>">
	...
	?>
	<div class="portfolio-items vamtam-cubeportfolio cbp portfolio-items" data-columns="<?php echo intval( $settings->columns ) ?>" data-options="<?php echo esc_attr( json_encode( $cube_options ) ) ?>" data-hidden-by-filters="<?php esc_attr_e( 'New items were loaded, but they are hidden because of your choice of filters', 'nex' ) ?>">
		<?php
