<?php  
    ...
    */
    ...
    $restricted = array('10000','61','63','65','66','74','75','77','78','80','83','84','86');
...
function formcraft_submit()
{
    ...
    while ($i<$nos)
    {
        // Prepare Email List for AutoReplies
...
{   
    ...
    if ($con[0]['mail_type']=='smtp')
    {
        ...
        foreach($rec as $send_to)
        {
            ...
            $std = "style='padding: 4px 8px 4px 0px; margin: 0; vertical-align: top; width: 120px; display: inline-block; font-size: 13px'";
            $std2 = "style='padding: 4px 8px 4px 0px; margin: 0; vertical-align: top; display: inline-block; font-size: 13px'";
            $i=0;
            $att=1;
            ...
            while ($i<$nos)
            {
                ...
                {
                    $mess .= "<div><span ".$std."><strong> ".$new[$i]['label']."</strong></span><span ".$std2.">".$new[$i]['value']."</span></div>";
                }
                else if ( $new[$i]['label']=='files' )
                {
                    $mess .= "<div><span ".$std."><strong>Attachment($att)</strong></span><span ".$std2."> <a href='".urlencode($new[$i]['value'])."'>".urlencode($new[$i]['value'])."</a></span></div>";
                    $att++;
                }
                else if ( $new[$i]['label']=='divider' )
                {
                    $mess .= "<div style='font-size: 16px; margin-top: 15px'> ".$new[$i]['value']."</div>";
                }
                else if ( $new[$i]['type']=='hidden' && $new[$i]['label']=='location' )
                {
                    $mess .= "<div style='font-size: 12px; display: block; padding: 4px 0px; margin-bottom: 10px; color: #888'> ".$new[$i]['value']."</div>";
                }
            ...
            $message = 
            '<html>
            <div style="padding: 8px; background-color: #eee; color: #555; font-size: 12px;">You have a new submission from FormCraft</div>
...
else
{
    ...
    while ($i<$nos)
    {
        if ( !(empty($new[$i]['type'])) && !($new[$i]['type']=='captcha') && !($new[$i]['type']=='hidden') && !($new[$i]['label']=='files') && !($new[$i]['label']=='divider') && !($new[$i]['type']=='radio') && !($new[$i]['type']=='check')  && !($new[$i]['type']=='smiley') && !($new[$i]['type']=='stars') && !($new[$i]['type']=='matrix') )
        {
            $mess .= "<div style='margin: 8px 0px;'><span ".$std.">".$new[$i]['label']."</span><span ".$std2."> ".$new[$i]['value']."</span></div>";
        }
        else if ( $new[$i]['label']=='files' )
        {
            $mess .= "<div style='margin: 8px 0px;'><span ".$std.">Attachment(".$att.")</span><span ".$std2."> ".urlencode($new[$i]['value'])."</span></div>";
            $att++;
        }
        else if ( $new[$i]['label']=='divider' )
        {
            $mess .= "<div style='font-size: 16px; margin-top: 20px; margin-bottom: 12px; padding: 4px 8px; border-bottom: 1px solid #ddd'> ".$new[$i]['value']."</div>";
        }
        else if ( $new[$i]['type']=='hidden' && $new[$i]['label']=='location' )
        {
            $mess .= "<div style='font-size: 12px; display: block; padding: 4px 8px; margin-bottom: 10px; color: #888'> ".$new[$i]['value']."</div>";
        }
        else if (  $new[$i]['type']=='radio' || $new[$i]['type']=='check' || $new[$i]['type']=='smiley' || $new[$i]['type']=='stars' || $new[$i]['type']=='matrix' )
        {
            if ( $new[$i]['value']==true )
...
function formcrafts_register_scripts () {
    ...
    wp_enqueue_style( 'facss', plugins_url( 'css/font-awesome/css/font-awesome2.min.css', __FILE__ ));
...
function widget($args, $instance)
{
    ...
    if (!(empty($id)))
    {
        ...
        <a href="#myModal" role="button" data-toggle="modal" id="'.$id.'_a" onClick="javascript:increment_form(this.id)">'.$temp_2.'</a>
...
function add_formcraft( $atts, $content = null ){
    ...
    if ($type=='popup')
    {
        ...
        <a href='#myModal' role='button' data-toggle='modal' id='".$id."_a' onClick='javascript:increment_form(this.id)' class='".$class."'>".$content."</a>
...
elseif ($type=='sticky') {
    ...
    <div id='nform_sticky' class='".$class." bootstrap'>
        <span id='".$id."_a'  title='Open/Close' onClick='javascript:increment_form(this.id)' class='sticky_toggle' style='background-color: $background; color: $text_color'>".$content."</span>
...
function formcraft( $id, $type = '', $opened = '0', $text = 'Click Here', $class = '', $background = '#eee', $text_color = '#333' ){
    ...
    {
        $temp= '
        <a href="#myModal" role="button" data-toggle="modal" id="'.$id.'_a" onClick="javascript:increment_form(this.id)" class='.$class.'>'.$text.'</a>
    ...
    elseif ($type=='sticky') {
        ...
        <div id='nform_sticky' class='".$class." bootstrap'>
            ...
            <span id='".$id."_a' onClick='javascript:increment_form(this.id)' class='sticky_toggle' style='background-color: $background; color: $text_color'>".$text."</span>
...
function formcraft_menu() {
    ...
    {
        add_menu_page( 'FormCraft Builder', 'FormCraft', 'read', 'survey_builder', 'formcraft_menu_options', plugins_url('formcraft/images/icon.png' ),'31.21' );
    }
    else
    {
        add_menu_page( 'FormCraft Builder', 'FormCraft', 'remove_users', 'survey_builder', 'formcraft_menu_options', plugins_url('formcraft/images/icon.png' ),'31.21' );
    }
}
