	...
	<div class="portfolio-item-wrapper">
		<?php if ( $has_details ) : ?>
			<div class="portfolio_details">
				...
				<a href="<?php echo esc_url( $single_url ) ?>" class="link-read-more"><?php esc_html_e( 'Read more &rarr;', 'nex' ); ?></a>
				<?php if ( $settings->lightbox_button === 'true' && 'mosaic' === $settings->layout && ( has_post_thumbnail() || ! empty( $video_url ) || ! empty( $gallery ) ) ) : ?>
					<div class="lightbox-wrapper">
						...
						?>
							<a href="<?php echo esc_url( $link ) ?>" class="cbp-lightbox icon theme" title="<?php esc_attr_e( 'View Media', 'nex' ) ?>" data-title="<?php the_title_attribute() ?>"><?php vamtam_icon( 'vamtam-theme-magnifier' ) ?></a>
						<?php endif ?>
					</div>
				<?php endif ?>
			</div>
