<div id="comment-<?php comment_ID() ?>" <?php comment_class( 'cbp-item' . ( $args['has_children'] ? 'has-children' : '' ) ) ?>>
	...
	<div class="comment-inner">
			...
			<?php
				if ( ( ! isset( $args['reply_allowed'] ) || $args['reply_allowed'] ) && ( $args['type'] == 'all' || get_comment_type() == 'comment' ) ) :
					comment_reply_link( array_merge( $args, array(
						'reply_text' => esc_html__( 'Reply', 'nex' ),
						'login_text' => esc_html__( 'Log in to reply.', 'nex' ),
						'depth'      => $depth,
		...
		<footer class="comment-footer">
			...
			<?php edit_comment_link( sprintf( '[%s]', esc_html__( 'Edit', 'nex' ) ) ) ?>
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<span class='unapproved'><?php esc_html_e( 'Your comment is awaiting moderation.', 'nex' ); ?></span>
			<?php endif ?>
		</footer>
	</div>
