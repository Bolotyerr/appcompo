...
wp_register_script( 'angularjss', plugins_url( 'js/angular-sanitize.min.js', __FILE__ ));
...
if ($conf[0]->bg_image==plugins_url('formcraft/images/wash.png')) 
	{$theme2 = 'active';}

if ($conf[0]->bg_image==plugins_url('formcraft/images/jean.png')) 
	{$theme3 = 'active';}

if ($conf[0]->bg_image==plugins_url('formcraft/images/debut.png')) 
	{$theme4 = 'active';}

if ($conf[0]->bg_image==plugins_url('formcraft/images/carbon.png')) 
	{$theme6 = 'active';}
...
wp_localize_script( 'mainjs', 'J', array( 'B' => $build, 'O' => $options, 'C' => $con, 'R' => $rec, 'I' => $ul, 'ide' => $id,  'countries' => plugins_url('/formcraft/data/countries.json'),  'states' => plugins_url('/formcraft/data/states.json'),  'languages' => plugins_url('/formcraft/data/languages.json')  ) );

// BootStrap JS
wp_enqueue_script( 'bootjs', plugins_url( 'bootstrap/js/bootstrap.min.js', __FILE__ ), array('jquery-ui-core','jquery-ui-widget','jquery-ui-slider','jquery-ui-sortable','jquery-ui-widget') );

wp_enqueue_script( 'datejs', plugins_url( 'datepicker/js/bootstrap-datepicker.js', __FILE__ ));
wp_enqueue_script( 'timejs', plugins_url( 'time/js/bootstrap-timepicker.min.js', __FILE__ ));

// Forms Main JS
wp_enqueue_script( 'formcraftjs', plugins_url( 'js/form.js', __FILE__ ));
...
wp_enqueue_script( 'formcraft-build-js', plugins_url( 'js/build.js', __FILE__ ));
wp_localize_script( 'formcraft-build-js', 'MyAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );

// jQuery and Angular UI JS
...
// File Upload JS
wp_enqueue_script( 'upload-js', plugins_url( 'file-upload/js/jquery.fileupload.js', __FILE__ ));
wp_enqueue_script( 'upload-iframe-js', plugins_url( 'file-upload/js/jquery.iframe-transport.js', __FILE__ ));
wp_enqueue_script( 'upload-widget-js', plugins_url( 'file-upload/js/vendor/jquery.ui.widget.js', __FILE__ ));


// ColorPicker
wp_enqueue_script( 'jquery-colorpicker-s', plugins_url( 'colorpicker/spectrum.js', __FILE__ ));
...
// Form Builder CSS
wp_enqueue_style( 'formcraft_builder_css', plugins_url( 'css/style.css', __FILE__ ));
...
// Form CSS
wp_enqueue_style( 'formcraft_forms_css', plugins_url( 'css/nform_style.css', __FILE__ ));


// ColorPicker
wp_enqueue_style( 'colorpicker_css', plugins_url( 'colorpicker/spectrum.css', __FILE__ ));
				...
				<div class="main_builder">
					<div class='build_affix' data-spy="affix" data-offset-top="0"><!-- Start of affixed Part -->
						...
						<div class='head_holder'>
							<a class='sbtn' href='?page=survey_builder'><i class='icon-chevron-left icon-white'></i> List of Forms</a>
											...
											<div class="accordion-inner l2">
												...
												<div class='global_holder'>
													...
													<div class='op_{{con[0].formpage}}'>
														...
														<input id="image_location" type="text" name="image_location" placeholder='Paste URL here or click Select' ng-model="con[0].formpage_image" style='width: 74%'/>
														<input  class="btn btn-primary upload_logo_formpage" type="button" value="Select Image" style='width: 25%'/>
														<br><br>
														...
														<div class='well well-small code'>
															<?php echo plugins_url().'/formcraft/form.php?id='.$_GET['id']; ?>
														</div>
													</div>
												...
												<div class='global_holder'>
													...
													Shortcode
													<div class='well well-small code'>[formcraft id='<?php echo $_GET['id']; ?>']</div>
													Widget
													<div class='well well-small code'>Go to <em class='surround'>Appearance -> Widgets -> formcraft</em></div>
													Use in Themes, and other places

													<div class='well well-small code'>&lt;?php formcraft(<?php echo $_GET['id']; ?>); ?&gt;</div>
									...
									<div class="accordion-inner l2">
										...
										<div class='global_holder'>
											...
											<em style='font-size: 12px; color: #888; line-height: 14px;'> 
												<br>
												(Enter your marketplace username here. You will receive a 30% commission on the first deposit / purchase made by user who clicked on this link. Read more <a href='http://codecanyon.net/make_money/affiliate_program' target="_blank">here</a>.)</em>
						...
						<div id="collapseTwo" class="form_accordion accordion-body collapse">
											...
											<div class="accordion-inner l2">
												...
												<div class='global_holder'>
													...
													<span class='btn-group ' data-toggle='buttons-radio'>
														<button type='button' class='btn btn-primary <? echo $theme1; ?> tool' ng-click='con[0].bg_image=""'>None</button>

														<button type='button' class='btn btn-primary <? echo $theme2; ?> tool' ng-click='con[0].bg_image="<?php echo plugins_url('formcraft/images/wash.png') ?>"'>Linen</button>

														<button type='button' class='btn btn-primary <? echo $theme3; ?> tool' ng-click='con[0].bg_image="<?php echo plugins_url('formcraft/images/jean.png') ?>"'>Jean</button>

														<button type='button' class='btn btn-primary <? echo $theme4; ?> tool' ng-click='con[0].bg_image="<?php echo plugins_url('formcraft/images/debut.png') ?>"'>Debut</button>

														<button type='button' class='btn btn-primary <? echo $theme6; ?> tool' ng-click='con[0].bg_image="<?php echo plugins_url('formcraft/images/carbon.png') ?>"'>Carbon</button>
													...
													<span class=''>
														...
														<input  class="btn btn-primary upload_logo_formpage" type="button" value="Select"/>
													</span>
										...
										<div id="globalThree" class="accordion-body collapse">										
											<div class="accordion-inner l2">
												...
												<div class='global_holder'>
													...
													<div class='global_holder'>
														<div class='gh_head'>Instructions</div> 
														<label for='ifs' class='option_text'>Font Size</label>
														<input type='number' min='1' id='ifs' ng-model='con[0].ifs' style='width: 60px'>
														<label for='ifc' class='option_text'>&nbsp; &nbsp;Font Color</label>
														<input class='cpicker' id='ifc' ng-model='con[0].ifc' style='width: 40px'>
													</div>
							...
							<div class='choose_from_main'>
								...
								<span class='choose_from btn-group'>
									...
									<ul class="dropdown-menu">
										<li><a ng-click="addEl('time')">TimePicker</a></li>
				...
				<td style='vertical-align: top'>
					<div class="preview_form">
						...
						<h4>You can <strong>drag</strong> and <strong>drop</strong> the fields to change their order.<br>Click to edit them.
							<a href='<?php echo plugins_url().'/formcraft/form.php?id='.$_GET['id'].'&preview=true'; ?>' target='_blank'>Live Preview (new window)</a>
						</h4>
						<div class='html_here'><?php $form_id = "123$_GET[id]"; ?><form class="nform bootstrap {{con[0].cl_hidden_fields}} {{con[0].user_save_form}} {{con[0].placeholder}} {{con[0].frame}} {{con[0].direction}} {{con[0].theme}} {{con[0].themev}} {{con[0].themef}} {{con[0].block_label}} star_{{con[0].show_star_validation}} {{con[0].flayout}}" action='javascript:submit_formcraft(<?php echo $form_id; ?>);' ng-style='{width: con[0].fw, borderRadius: con[0].fr+"px", MozBorderRadius: con[0].fr+"px", WebkitBorderRadius: con[0].fr+"px", fontFamily: con[0].formfamily, backgroundImage: "url("+con[0].bg_image+")", background: con[0].bg_transparent}' id='<?php echo $form_id; ?>'><input type='hidden' class='form_id' val='<?php echo $_GET['id']; ?>' ng-model='con[0].form_main_id'><input type='hidden' class='getlocation' val='' name='location_hidden__0_0_1000_'><div id='fe_title' class='nform_li form_title {{con[0].theme}}' ng-style='{fontSize: con[0].ft_px+"px", borderTopLeftRadius: con[0].fr+"px", MozBorderTopLeftRadius: con[0].fr+"px", WebkitBorderTopLeftRadius: con[0].fr+"px", borderTopRightRadius: con[0].fr+"px", MozBorderTopRightRadius: con[0].fr+"px", WebkitBorderTopRightRadius: con[0].fr+"px", color: con[0].ft_co, backgroundColor: con[0].ft_bg, fontFamily: con[0].tfamily, fontWeight: con[0].tbold, backgroundImage: "url("+con[0].bg_image+")", background: con[0].bg_transparent, textAlign: con[0].ftalign}'>{{con[0].form_title}}</div><ul class='form_ul clearfix {{con[0].theme}}' id='form_ul'><li ng-repeat="el in build" ng-style='{paddingBottom: con[0].space+"px", paddingTop: con[0].space+"px"}' id='fe_{{$index}}' class='nform_li {{el.default}} {{el.inline}} {{con[0].field_align}} {{el.li_class}} fe_{{$index}}'><div compile="el.el_f" ng-style='{marginBottom: el.divspa, marginTop: el.divspa}'></div><span class='element_id'>{{$index}}</span></ul><div id='fe_submit' class='form_submit' style='position: relative; display: block'><span style='text-align: center; display: inline-block'><a class='ref_link' ng-href='http://codecanyon.net/item/formcraft-premium-wordpress-form-builder/5335056?ref={{con[0].ruser}}' compile="con[0].rlink" target='_blank' title='Opens in a new tab'></a></span><div class='res_div'><span class='nform_res'></span></div></div></form>
					</div>
